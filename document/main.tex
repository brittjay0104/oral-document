...
% TODO more here on communication theory
...
% TODO more here on similarities between verbal and tool comm (or explicit mapping to comm model?)

\subsection{Why do developers not use or have difficulty using program analysis tools?}
...
\subsection{Why don't developers use static analysis tools to find bugs?}\label{subsec:s1}
...
Finally, participants elaborated a number of ideas for how current static analysis tools could improve. 
...
\subsection{Why do developers have difficulty interpreting tool notifications?}\label{subsec:s2}
...
\subsection{Can we use developer experiences to predict knowledge?}\label{subsec:s3}
...
\begin{labeling}{hypotheses}
	...
	\item [H\textsubscript{2}] Concept-specific code increases model accuracy and precision compared a model that uses lines of code (LOC) only.
\end{labeling}

\subsubsection{Approach.}
\vspace{0.5em}
\noindent\textit{Assessing \& Encapsulating Conceptual Knowledge.}
To assess \& encapsulate developer knowledge as the dependent variable for our models, I borrowed from computer science education literature and developed a concept inventory~\cite{tew2010assessing}. 
...
\subsection{Does my prototype concept knowledge model scale?} \label{subsec:s4}
...
\subsection{Are adaptive, knowledge-based notifications more effective than one-size-fits-all notifications?} \label{subsec:s5}
\subsubsection{Study rationale.}
\subsubsection{Research questions.}
...
\begin{labeling}{questions}
	...
	\item [RQ3] Are adaptive, knowledge-based notifications more effective than one-size-fits-all notifications?
\end{labeling}
